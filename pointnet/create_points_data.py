"""Build an augmented dataset of hand-landmark point clouds (.npy) from the ASL alphabet images."""
import glob
import os
import random
...
from transforms_3d import GaussianNoise, Rotate, Scale
    ...
    if points_raw is not None:
        ...
        if "scale" in augmentations:
            for _ in range(4):
                ...
                np.save(os.path.join(dest_path, label, name.split(".")[0] + f"_Scale_{factor:.2f}_Axis_{axis}"), scaled)
                count += 1
